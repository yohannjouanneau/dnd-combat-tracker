...
export default function TokenPanel({
  ...
  return (
    <div className="absolute bottom-4 left-3 z-10 w-72 bg-panel-bg border border-border-primary rounded-xl p-4 flex flex-col gap-3 shadow-xl max-h-[calc(100vh-5rem)] overflow-y-auto">
      <IconButton
        variant="ghost"
        onClick={onClose}
        ...
        {tokens.map((token) => (
          ...
          >
            ...
            <button
              ...
              title={
                token.hidden ? t("token.hiddenTitle") : t("token.visibleTitle")
              }
            >
              {token.hidden ? (
      ...
      {selectedToken && (
        <>
          ...
          {/* Label */}
          <div className="flex flex-col gap-1">
            <span className="text-xs text-text-muted">{t("token.label")}</span>
            <input
              type="text"
          ...
          <div className="flex items-center justify-between gap-3">
            <span className="text-xs text-text-muted">{t("token.color")}</span>
            <input
              type="color"
          ...
          <div className="flex flex-col gap-2">
            <span className="text-xs text-text-muted">{t("token.image")}</span>
          ...
          <div className="flex flex-col gap-1.5">
            <div className="flex items-center justify-between">
              <span className="text-xs text-text-muted">{t("token.size")}</span>
