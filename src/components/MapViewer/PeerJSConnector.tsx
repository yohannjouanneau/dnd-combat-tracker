...
export default function PeerJSConnector({
  ...
  // If the DM is already hosting, open directly on the dm-waiting step
  const [step, setStep] = useState<Step>(() =>
    dmSession?.isHosting ? { kind: "dm-waiting" } : { kind: "choosing" },
  );
      ...
      <div className="bg-panel-bg border border-border-primary rounded-xl p-6 w-full max-w-sm flex flex-col gap-5 relative">
        ...
        {step.kind === "dm-waiting" && (
          <div className="flex flex-col items-center gap-4">
            {networkWarning}
            {dmSession && dmSession.connectedCount > 0 ? (
              <p className="text-green-400 text-sm font-semibold">
                {t("connector.playersConnected", {
                  count: dmSession.connectedCount,
                })}
              </p>
            ) : (
              <>
                ...
                <p className="text-text-muted text-sm">
                  {dmSession?.roomCode
                    ? t("connector.waitingForFirstPlayer")
            ...
            <div className="w-full">
              ...
              <div className="flex items-center gap-2">
                ...
                {dmSession?.roomCode && (
                  <IconButton
                    onClick={() => copyRoomCode(dmSession.roomCode!)}
