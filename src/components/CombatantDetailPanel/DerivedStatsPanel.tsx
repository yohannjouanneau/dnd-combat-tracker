...
export default function DerivedStatsPanel({
  ...
  const profBonus = getEffectiveProficiencyBonus(
    combatant.level,
    combatant.proficiencyBonus,
  );
  const spellcastingScore = combatant.spellcastingAbility
    ? { int: combatant.int, wis: combatant.wis, cha: combatant.cha }[
    ...
    return (
      <div className="space-y-2 mt-2">
        ...
        <div className="grid grid-cols-3 gap-1">
          {(["perception", "insight", "investigation"] as const).map(
            (skill) => {
              const profKey = `${skill}Proficiency` as
                | "perceptionProficiency"
                | "insightProficiency"
                | "investigationProficiency";
              const abilityMap = {
                perception: combatant.wis,
                insight: combatant.wis,
                investigation: combatant.int,
              };
              const score = getPassiveScore(
                abilityMap[skill],
                profBonus,
                combatant[profKey],
              );
              ...
              return (
                <div
                  key={skill}
                  className="bg-panel-secondary rounded px-1 py-1.5 flex flex-col items-center"
                >
      ...
      <div className="grid grid-cols-3 gap-2 mb-3">
        {(["perception", "insight", "investigation"] as const).map((skill) => {
          ...
          return (
            <div
              key={skill}
              className="bg-panel-secondary rounded-lg p-2 flex flex-col items-center"
            >
