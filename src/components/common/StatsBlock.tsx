...
import DerivedStatsPanel, { type DerivedStatsInput } from "../CombatantDetailPanel/DerivedStatsPanel";
...
export default function StatsBlock({ hp, maxHp, ac, initiative, scores, derivedStats, mode = "large" }: Props) {
      ...
      <div className="space-y-2">
        ...
        <div className="flex gap-2">
          <div className="flex items-center gap-1.5 bg-panel-secondary rounded px-2.5 py-1.5 flex-1 justify-center">
            ...
            <span className={`text-sm font-bold ${getHpColorClass(hp ?? 0, maxHp ?? 1)}`}>
              {hp ?? 0}/{maxHp ?? 0}
            </span>
          </div>
          <div className="flex items-center gap-1.5 bg-panel-secondary rounded px-2.5 py-1.5 flex-1 justify-center">
          ...
          {initiative !== undefined && (
            <div className="flex items-center gap-1.5 bg-panel-secondary rounded px-2.5 py-1.5 flex-1 justify-center">
              ...
              <span className="text-sm font-bold text-blue-400">{initiative >= 0 ? `+${initiative}` : initiative}</span>
            </div>
          )}
        </div>
        ...
        {scores && (
          <div className="grid grid-cols-6 gap-1">
            {ABILITY_KEYS.map(key => {
              ...
              return (
                <div key={key} className="bg-panel-secondary rounded px-1 py-1.5 text-center">
                  <div className="text-xs text-text-muted leading-none">{ABILITY_LABELS[key]}</div>
                  <div className="text-sm font-semibold text-text-primary leading-none mt-1">{val}</div>
                  <div className="text-xs text-blue-400 leading-none mt-0.5">{getAbilityModifier(val)}</div>
                </div>
              );
            })}
          </div>
        )}
        {derivedStats && <DerivedStatsPanel combatant={derivedStats} mode="compact" />}
      </div>
    );
  }
    ...
    <div className="space-y-4">
      ...
      <div className="flex gap-2 md:gap-4">
        <div className="bg-panel-secondary rounded-lg p-2 md:p-4 flex-1 flex flex-col items-center">
          ...
          <div className={`text-2xl md:text-3xl font-bold ${getHpColorClass(hp ?? 0, maxHp ?? 1)}`}>
            {hp ?? 0} / {maxHp ?? 0}
          </div>
        </div>
      ...
      {/* Ability scores */}
      {scores && (
        <AbilityScore type="combatant_details" scores={scores} />
      )}
